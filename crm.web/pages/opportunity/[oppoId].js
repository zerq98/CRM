                    ...
                    <div className="flex md:flex-row xs:flex-col border-1 h-80p w-100p p-2 overflow-y-scroll overscroll-contain scrollbar-hide md:space-x-2">
                        <div className="flex flex-col md:w-30p xs:w-100p h-100p space-y-0.5">
                          ...
                          onChange={(e)=>{
                            ...
                            {oppo.lead!==''?<option disabled></option>:<option selected disabled></option>}
                            {leadList.map((d,key)=>{
                                if(d===oppo.lead){
                        ...
                        <div className="flex flex-col md:w-30p xs:w-100p h-20p space-y-0.5">
                          ...
                          onChange={(e)=>{
                            ...
                            {oppo.status!==''?<option disabled></option>:<option selected disabled></option>}
                            {statuses.map((d,key)=>{
                                if(d===oppo.status){
                                  return <option selected>{d}</option>
                                }else{
                        ...
                        <div className="flex flex-col md:w-30p xs:w-100p h-20p space-y-0.5">
                          ...
                          onChange={(e)=>{
                            ...
                            {oppo.trader!==''?<option disabled></option>:<option selected disabled></option>}
...
export async function getServerSideProps(context) {
    ...
    if (sess) {
      ...
      const res = await fetch(server+"Opportunity/GetOpportunity?opportunityId="+context.params.oppoId, {
            method: 'GET',
            headers: {
            accept: '*/*',
            "Content-Type": "application/json",
